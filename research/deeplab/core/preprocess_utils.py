...
def rotate(tensor_list, prob=0.5):
    ...
    def rot():
        ...
        for tensor in tensor_list:
            ...
            rotated_image = tf.contrib.image.rotate(
                tf.expand_dims(tensor, axis=0), angles, interpolation='BILINEAR', name=None)
            flipped.append(tf.squeeze(rotated_image, axis=[0]))

        return flipped

    is_rotate = tf.less_equal(random_value, prob)
    outputs = tf.cond(is_rotate, rot, lambda: tensor_list)
    return outputs
